...
public class Main {
	...
	public int q1() {
		return jogadoresList.stream().filter(Objects::nonNull).collect(Collectors.groupingBy(Jogador::getNationality)).size();
	}

	// Quantos clubes (coluna `club`) diferentes existem no arquivo?
	// Obs: Existem jogadores sem clube.
    public int q2() {
        return jogadoresList.stream().filter(Objects::nonNull).collect(Collectors.groupingBy(Jogador::getClub)).size()-1;
	...
	public Map<Integer, Integer> q6() {
		...
		jogadoresList.stream().map(Jogador::getAge).distinct().forEach(key -> lista.put(key, jogadoresList.parallelStream()
